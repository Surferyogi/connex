...
export async function clearOfflineData(userId) {
  ...
  try {
    ...
    dbPromise = null; // reopen on next use (a closed handle would fail every call)
  }
}
...
// `variant` prefixes the cache key so a thumbnail and the full photo of the
// same path are stored separately ("" = full photo, "thumb:" = list tile).
// `strict` (thumbnails): return null instead of an unverified signed URL when
// the bytes couldn't be fetched, so the caller can fall back to the full photo.
export async function cachedImageUrl(path, signedUrl, variant = "", strict = false) {
  if (!path) return null;
  ...
  try {
    ...
    if (!res.ok) return strict ? null : signed;
...
export async function pruneImages(keepPaths) {
  try {
    ...
    await idbDelete(keys.filter((k) => !keep.has(k) && !keep.has(k.replace(/^thumb:/, ""))));
  } catch {
    /* ignore */
  }
}
